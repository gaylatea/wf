...
Pebble.addEventListener('showConfiguration',
                        function(e) { Pebble.openURL(clay.generateUrl()); });
...
var updateTraffic = function() {
  ...
  navigator.geolocation.getCurrentPosition(
      function(pos) {
        destinations = settings.l1lat + ',' + settings.l1long + '|' +
                       settings.l2lat + ',' + settings.l2long;
        var params = {
          key : settings.apikey,
          departure_time : 'now',
        ...
        xhrRequest(url, 'GET', function(responseText) {
          ...
          var l1_minutes =
              Math.round(data[0]['duration_in_traffic']['value'] / 60);
          var l2_minutes =
              Math.round(data[1]['duration_in_traffic']['value'] / 60);

          // Send information to the watch.
          Pebble.sendAppMessage({0 : l1_minutes, 1 : l2_minutes},
      ...
      function(err) { console.log('Location services not available.'); },
